namespace Blockpit.Listener.PollToPublishTasks
{
    ...
    public class BlockCypherBtcPollToPublishTask(
        ILog log,
        IMediator mediator,
        ...
        public async Task StartAsync()
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    var response = await HttpJsonHelper.GetAsync<BlockCypherBtc>(url, jsonSerializerOptions, ignoreSsl);
                    ...
                    {
                        counterService.AddEvent("BTCPollRepeat");
                    ...
                    counterService.AddEvent("BTCPoll");

                    log.Info($"Url: {url}; Mediator finished.");
                }
                catch (TransactionRollbackIdempotency)
                {
                    counterService.AddEvent("BTCPollRepeatMediator");

                    log.Info($"Url: {url}; is a repeat;");
                }
                catch (Exception e)
                {
                    counterService.AddEvent("BTCPollErrors");
