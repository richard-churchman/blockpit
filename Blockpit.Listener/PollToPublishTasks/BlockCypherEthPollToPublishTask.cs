namespace Blockpit.Listener.PollToPublishTasks
{
    ...
    public class BlockCypherEthPollToPublishTask(
        ILog log,
        IMediator mediator,
        ...
        public async Task StartAsync()
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    var response = await HttpJsonHelper.GetAsync<BlockCypherEth>(url, jsonSerializerOptions, ignoreSsl);
                    ...
                    if (response is
                        {
                            HighGasPrice: not null, MediumGasPrice: not null, LowGasPrice: not null,
                            HighPriorityFee: not null, MediumPriorityFee: not null, LowPriorityFee: not null, BaseFee: not null
                        })
                    {
                        log.Info($"Url: {url} has Gas");

                        block.SetGasFees(
                            response.HighGasPrice.Value,
                            response.MediumGasPrice.Value,
                            response.LowGasPrice.Value,
                            response.HighPriorityFee.Value,
                            response.MediumPriorityFee.Value,
                            response.LowPriorityFee.Value,
                            response.BaseFee.Value
                        );
                    }
                    ...
                    counterService.AddEvent("ETHPoll");

                    log.Info($"Url: {url} Mediator finished.");
                }
                catch (TransactionRollbackIdempotency)
                {
                    counterService.AddEvent("ETHPollRepeatMediator");

                    log.Info($"Url: {url}; is a repeat;");
                }
                catch (Exception e)
                {
                    counterService.AddEvent("ETHPollErrors");
